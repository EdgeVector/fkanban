...
const shimPath = resolve(here, "../bin/fkanban");
...
describe("bin/fkanban bun resolution", () => {
  afterEach(() => {
  ...
  test("falls back to $HOME/.bun/bin/bun when GUI PATH omits bun", async () => {
  ...
  test("reports a clear error when bun is unavailable", async () => {
  ...
  test("starts MCP via the shim when GUI PATH omits bun", async () => {
    const home = homeWithFallbackBun();
